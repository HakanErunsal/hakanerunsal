...
The Last Line has passed ten thousand downloads on Google Play, and player reports keep surfacing what my test devices miss. It is free to play; subscriptions and in-app purchases run through RevenueCat, which I wrapped behind a single Unreal subsystem and published as its own plugin, the [RevenueCat Bridge](/projects/RevenueCatBridge). Apple is reviewing the iOS build. Five months on no salary was harder than any bug, but it shipped, and the oversized update is next on the list.
