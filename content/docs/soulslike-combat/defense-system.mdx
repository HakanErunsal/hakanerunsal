...
description: "Give a pawn a guard: create a Defense Rule Set, author one response, and an incoming hit is parried, blocked or ignored before it reaches health."
date: 2026-08-17
published: true
...
<QuickStep title="Create a Defense Rule Set">

Right-click in the Content Browser, **SEC**, **Defense Rule Set**.
...
On the asset, press **+** on **Responses** and pick **SEC Block**. Set **Required Tags** to the tag the guard holds while it is up, and **Result Tag** to `SEC.Defense.Block` so the attacker learns what happened.

<UeSetPropertiesScene
  title="DA_Defense_KnightArmour"
  section="Block"
  rows={[
...
<QuickStep title="Point the Defense component at it">

Select the pawn's **SEC Defense** component and set **Defense Set** to the asset.

<UeSetPropertiesScene
  title="SEC Defense"
  section="SEC|Defense"
  rows={[
    { name: "Defense Set", value: "DA_Defense_KnightArmour", kind: "asset", accent: "#C91D55" }
  ]}
  caption="Defense Set answers after everything authored directly on the component, which ships empty."
/>
...
Set more than one Defense component's **Defense Set** to the same asset, `DA_Defense_KnightArmour` from the walkthrough above or a copy authored for another archetype, to give every pawn carrying it the same answers. Edit the asset once and every pawn pointed at it picks up the change.
