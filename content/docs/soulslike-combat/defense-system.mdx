...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Arc Half Angle Degrees", value: "60", from: "60" },
    { name: "Damage Multiplier", value: "0.2", from: "0.2" },
    { name: "Result Tag", value: "SEC.Defense.Block", kind: "text" }
  ]}
  caption="Required Tags is the only field with nothing in it. The arc and the multiplier ship ready to use."
/>
...
Filter the Output Log to **LogSECCore**. The defense path names each problem once per class rather than every frame, and the four it reports cover most of what goes wrong: a response that charges a vital the pawn has no component for, a response that answered without a **Result Tag**, a pawn with no health pool to take the damage off, and an attack listing the health pool as an extra damage channel.
...
- **Handle Incoming Damage is not implemented.** Without the **SEC Damageable** interface on the pawn, **Deal Damage** falls back to Unreal's point damage and the Defense component never runs.
