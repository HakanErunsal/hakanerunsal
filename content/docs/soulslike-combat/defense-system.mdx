...
In the pawn's **Class Settings**, add the **SEC Damageable** interface, then implement **Apply Damage** with one node.

<DefenseResolveGraph />

**Resolve and Apply Incoming Damage** is the whole answer in one call: run the responses, charge what they cost, take what survived off the pawn's health pool, and report back with **Was Fatal** set when that emptied it. A pawn with no vitals component resolves the hit normally, takes nothing off anything, and names itself once in the Output Log.

Two variations cover the rest:

- **A pawn wired through Event Any Damage.** Call **Resolve Incoming Damage (Any Damage Event)** instead. It takes loose numbers and works the blow's direction out from where the attacker is standing, so a directional guard still has a bearing to measure against.
...
<Callout type="info">
Enemies deriving from `AEnemyCharacterBase` answer Apply Damage in C++ already, through a Defense component named **Guard**. Author responses on it and the enemy is done.
</Callout>
...
Most situational at the top. The resistance sits at the bottom, on the component or in a shared set, with **Continue After Claim** ticked so it trims whatever the guards above it let through.
...
```cpp
FSECDefenseResult Defense = USECDefenseStatics::EvaluateDefense(
    Defender, Guard->GetEffectiveResponses(), DamageInfo, Guard);

USECDefenseStatics::ApplyDefenseResult(Defense, Guard);   // charges the costs, server only

FSECDamageResult Result;
USECDefenseStatics::FillDamageResult(Defense, Result);    // DamageApplied + ResultTags
```
...
Charging a cost and draining health both need the authority, so run **Resolve and Apply Incoming Damage** on the server. A call anywhere else charges nothing and returns 0.
