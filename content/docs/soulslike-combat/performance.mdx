...
| **Reaction scoring** | Once per incoming stimulus that passes its filters | Gates and scores the reactions a trigger rule allows | Rules matched, times reactions per rule |
...
Territory, tokens and reactions do no work until something calls them, so an enemy standing at its post between legs costs the movement pass and the brain tick and nothing else.
