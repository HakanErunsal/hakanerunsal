...
| **Enable Avoidance** | on | Untick it and the sphere overlap for neighbouring pawns stops. Worth it for solo bosses and sparse fields where enemies never pack. |
...
Start with **Num Samples**. It multiplies three of the four movement stats at once, so halving it moves the largest number on the profile. Setting **Avoidance Weight** to 0 skips the overlap query as well, without unticking anything.
...
|--------|------|---------------|
| **Movement** | Every tick that a brain or a **Poll AI Movement** task asks for it | Scores **Num Samples** directions and picks a winner |
...
| Action RPG pack | 10 to 20 | Direction sampling, with avoidance climbing as they close on you | Raise **Avoidance Radius** rather than cutting samples, so fewer neighbours qualify. |
...
**Role Reassignment Interval** (8 s) is how often global role assignment re-runs. It lives in **Project Settings > Soulslike Enemy Combat > Combat Roles > Timing**, so it is one value for the project. Set it to 0 and the periodic pass never starts, leaving role assignment to whatever asks for it.
...
**Sight Radius** (1500 cm) is the biggest lever, because the sight sense tests every registered stimulus source inside it. Cut it to what the encounter actually needs. **Peripheral Vision Half Angle** (90 degrees, a 180 degree cone) narrows what qualifies before a trace happens.
...
While a swing runs, each registered socket sweeps once per frame, sub-stepped so a fast weapon cannot skip past a thin target between frames. **Sub Step Distance** (10 cm) is the longest gap one sweep step may cover, and **Max Sub Steps** (20) caps the count however many the speed asks for.
...
- **Staggered nav sweeps.** **Nav Check Interval** and **Nav Stagger Stride** spread full nav sweeps across frames, so a whole pack never sweeps on the same one.
- **Timer-based roles.** Role assignment runs on its interval, off a timer, never per frame.
...
The **Movement Evaluator** component does not tick. Something has to ask it for a pass, which is what the **SEC Brain** component and the StateTree **Poll AI Movement** task do each tick.
