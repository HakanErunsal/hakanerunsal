...
| Field | Sets |
|-------|------|
| **Radius** | How far the warning reaches, in centimetres from where it starts. A pawn outside it learns nothing is coming. Starts at 500. |
| **Arc Half Angle Degrees** | Half the width of the arc the warning covers, in degrees to either side of the facing. 180 warns all round, 45 warns a narrow cone straight ahead. Starts at 180. |
| **Arc Yaw Offset Degrees** | Turn the arc away from straight ahead, in degrees. Use 90 for a swing coming in from the right so the warning points where the blade goes. Starts at 0. |
| **Team Filter** | Who gets warned. Enemies Only is what an attack wants, so allies standing in the arc are not startled by their own side. Team ids come from the **SEC Team Interface**, and an actor without one counts as neutral. |
| **Notice Chance** | How likely each pawn in range is to notice, from 0 to 1. Rolled once per pawn, so at 0.6 roughly three enemies in five spot the wind-up and the rest are caught out. Starts at 1, which warns the whole arc. |
| **Draw Debug** | Draw the reach and the arc in the world for a moment, to check the shape against the animation. |
...
| **Condition** | An extra condition on top of the tags, for something the tags cannot express such as the pawn's own health. Leave it empty and the tags decide on their own, which covers most rules. |
...
### 4. Make the parry win only from the front
...
A blow from the side fails the parry's gate and the dodge wins instead, with no second rule. That is the division of labour: the rule says something is coming, the gate says whether this reaction is the right answer to it.
...
Writing a filter or a condition of your own is a Blueprint subclass either way: **SEC Stimulus Filter** answers **Can Sense**, and **SEC Reaction Trigger** answers **Should Fire** with the scoring context of the pawn, the target, and whatever set it off. Keep both stateless, since every AI using the asset shares one instance.
