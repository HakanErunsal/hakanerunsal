...
Switch **Shape** to **Cone** for a directional swing, set **Radius** to the attack's reach, and set **Half Angle Degrees** to the wedge it threatens.
...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Radius", value: "600", from: "500" },
    { name: "Half Angle Degrees", value: "60", from: "60" }
  ]}
  caption="Radius is the reach in centimetres. Half Angle Degrees is measured to either side of where the attack is aimed."
/>
...
<Callout type="info">
The notify works out how long is left by finding the next **SEC Melee Trace Window** further along the montage and dividing by the play rate in force, so retiming the animation keeps it right. A montage with neither a trace window nor a **Time To Impact Override** warns once in the Output Log and reports 0 seconds left. See [Melee Trace](/docs/soulslike-combat/melee-trace).
</Callout>
...
<Callout type="info">
Nothing hands an action a warning, so one of these four gates added to an action has nothing to judge and passes. They belong on the reaction side.
</Callout>
...
A fireball warns what is in front of it on spawn: pass the projectile as **Source**, its velocity as **Facing**, and a narrow cone as the shape. **Facing** falls back to the source actor's forward vector when left at zero.
...
Draws the covered ground and the facing at the origin, then one sphere at every pawn the announcement reached, coloured by what happened, with a line back to the origin and a label naming the outcome. Markers last two seconds, and the same lines go to the Output Log.
...
- **The notify sits past the trace window.** It fires at the point you put it, so a notify placed after the blow warns nobody in time.
...
- **A Time To Impact Gate refused it.** A warning that reports 0 seconds left fails any **Min Seconds** above 0. That is the gate doing its job on a hit that already landed, and a mistake on a wind-up whose montage carries no trace window.
...
One shape instance answers for each pawn a warning reaches, and a notify is shared by every montage carrying it, so keep a subclass free of stored state.
</Collapsible>
