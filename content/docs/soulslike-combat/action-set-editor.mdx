...
Double-click any Action Set and it opens as cards. If you have none yet, right-click in the Content Browser, **SEC**, **Action Set**.
...
The **Watching** picker fills from AI whose active Action Set is this asset, so an enemy reaches it through its **Enemy AI Config**, and it scores nothing until a combat target is registered. A picker stuck on **None** while the game runs is one of those two. See [Action System](/docs/soulslike-combat/action-system) and [Getting Started](/docs/soulslike-combat/getting-started).
</Callout>
...
Rows appear only when they have something to say, so a plain attack card stays short. Hover the card for the same figures as sentences, plus one line per chain link and the bonus that follow-up receives.
...
- An empty **Action ID**, or one another action already uses.
- A chain link naming an action this set does not define.
- **Manual Only** set on a card that something wires into, since a chain into it changes nothing.
...
Saving the asset runs validation into the **Soulslike Enemy Combat** message log, which names any action that has nothing to run.
