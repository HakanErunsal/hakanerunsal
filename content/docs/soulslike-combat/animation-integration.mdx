...
2. In the pawn's **Handle Incoming Damage**, check for that tag. While it is held, skip the health change and leave **`DamageApplied`** at zero, with `SEC.Defense.Dodge` in **`ResultTags`**.
