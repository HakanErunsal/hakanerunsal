...
> Multi-target support for co-op, versus, and dynamic combat scenarios. AI selects, switches, and redistributes targets on its own.
...
> Single-player works out of the box. Zero configuration needed. AI assigns to the first registered target.
...
The system tracks **multiple combat targets** rather than a single "player" reference:
...
| **AwarenessFilteredTargetSelector** | Wraps another selector; only passes targets the AI has actually sensed | Stealth / "fights only what it has seen" |

Selectors are **data-driven**. Configure them per-AI via `EnemyAIConfig` or project-wide defaults.

> **AwarenessFilteredTargetSelector** pairs with the [Awareness System](/docs/soulslike-combat/awareness-system). Its `MinRequiredState` sets the disengage feel: **Detected** = break off and investigate the moment sight is lost (recommended for stealth/patrol enemies); **Lost** (default) = keep chasing through the grace window. It delegates to an `InnerSelector` (or nearest) once the list is filtered.

### Lazy Target Acquisition

Enemies can spawn before any target exists:
...
The lazy acquisition pattern handles the classic "AI spawns before player" race condition. Use `RegisterCombatTarget(Player, /*bAutoAssignUnassigned=*/ true)` to trigger immediate assignment of waiting AI.
