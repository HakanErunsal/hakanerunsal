...
Bind to the `SECCombatControllerComponent`'s delegate on your AI Controller:
...
This resolves the AI's `TargetSelector` from its `EnemyAIConfig`, falls back to the project default, removes the AI's own pawn, removes friendly targets, and returns the selector's pick.
...
By default, the reassignment timer only re-evaluates **roles**, not targets. In multi-target scenarios where targets are moving around, you may want AI to periodically reconsider which target is best.

Enable periodic target re-evaluation in **Project Settings → Plugins → Soulslike Enemy Combat → Timing**:
...
The subsystem automatically filters targets based on team affinity. Any actor implementing `ISECTeamInterface` (or `IGenericTeamAgentInterface`) has a TeamId. The subsystem uses `USECTeamHelpers::AreFriendly()` to exclude friendly targets from every assignment path:
...
This is fully backward compatible. Existing setups without teams are unaffected since all actors without a team interface are treated as neutral.
