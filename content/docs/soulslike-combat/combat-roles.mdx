...
Enemies don't all attack at once. Each gets assigned a **role**:
...
**Role Evaluators** (`URoleEvaluator` subclasses) decide who becomes the next Attacker. These are instanced UObjects that score each AI on a 0.0-1.0 scale:
...
> `OnCombatantsOrphaned` fires when a target unregisters. The subsystem resets orphaned combatants to `SEC.Role.None`. Bind to this delegate to reassign them to a new target.
...
If you use `AEnemyControllerBase`, role syncing is handled for you. Its brain (`USECBrainComponent`) queries the current role and resolves the appropriate ActionSet and MovementProfile from the `EnemyAIConfig`. This works whether the brain runs a StateTree (when `AIConfig.DefaultStateTree` is set) or its native combat loop (when that field is empty).
