...
- **`RegisterCombatTarget`**: adds the actor to the target pool.
  - **`bAutoAssignUnassigned`**: calls `ForceReassignment` and `EvaluateRoleDistribution` straight away. A combatant standing idle picks a target through its own Target Filter and Target Selector whether this is set or not, so an enemy whose team rules turn the target aside stays idle.
...
3. The combatant receives a target from `InitialTarget` (runtime-only), `TargetSelector`, or, with no selector configured, the first registered target its team rules allow it to fight.
...
2. **`SyncStateForCombatRole`** pushes the matching action set and reaction set, asks the equipped weapon what it gives for the new role, and applies the movement profile.
...
| Function | Syncs |
|----------|-------|
| `SyncAllForCombatRole(Controller, Pawn, RoleTag, AIConfig)` | Action + reaction + weapon sets + movement |
