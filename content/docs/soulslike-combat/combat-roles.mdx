...
**Customizing Sync Behavior:** You can subclass `SECCombatControllerComponent` in Blueprint and override `SyncStateForCombatRole` to add custom logic when roles change (e.g., switch animation sets, trigger visual effects, load a different StateTree, or notify other systems). Call the parent function to preserve the default action set and movement profile sync.

```cpp
// Override in your Blueprint subclass of SECCombatControllerComponent:
void UMyCombatController::SyncStateForCombatRole_Implementation(const FGameplayTag& RoleTag)
{
    // Call parent to keep default ActionSet + MovementProfile sync
    Super::SyncStateForCombatRole_Implementation(RoleTag);

    // Your custom sync logic here
    if (RoleTag.MatchesTag(FGameplayTag::RequestGameplayTag("SEC.Role.Attacker")))
    {
        // Switch to aggressive animation blueprint, trigger FX, etc.
    }
}
```

Similarly, you can override `HandleCombatTargetLost` to customize behavior when a target is destroyed or unregistered.
...
For bosses or scripted AI that should adopt a combat role **without participating in the subsystem's slot coordination**, use `ApplyLocalCombatRole` on the `SECCombatControllerComponent`.

### Why Local Roles?

The subsystem enforces slot limits (e.g., max 2 Attackers). A boss shouldn't consume one of those slots — it should always behave as an Attacker regardless of what the other enemies are doing. Local roles give the boss:

- ✅ Role-based ActionSet and MovementProfile sync (identical to subsystem-assigned roles)
- ✅ `OnCombatRoleChanged` delegate fires normally
- ✅ Pawn's `SECCombatRoleComponent` updates for client replication
- ❌ Does **not** consume subsystem slots
- ❌ Does **not** participate in role redistribution

### Setup

1. Set `bAutoRegisterForCombatRoles` to `false` on the boss controller's `SECCombatControllerComponent` (or in its `EnemyAIConfig`)
2. Call `ApplyLocalCombatRole` when the boss should start fighting:

```cpp
// In your Boss AI Controller or Blueprint:
USECCombatControllerComponent* CombatComp = FindComponentByClass<USECCombatControllerComponent>();
CombatComp->ApplyLocalCombatRole(FGameplayTag::RequestGameplayTag("SEC.Role.Attacker"));
```

Or in Blueprint: call **Apply Local Combat Role** on the component with the desired role tag.

### Combining With the Subsystem

If you call `ApplyLocalCombatRole` on an AI that **is** registered with the subsystem, a warning will be logged. The subsystem's next evaluation cycle may overwrite the locally applied role. To avoid this, either:
- Don't register the AI (`bAutoRegisterForCombatRoles = false`)
- Or unregister it first, then apply the local role

### OnCombatRoleSystemReady Delegate

If you need to call `ForceAssignRole` immediately after spawning (for AI that *does* register), bind to `OnCombatRoleSystemReady`. This delegate fires after the subsystem accepts the registration, guaranteeing it's safe to call subsystem APIs:

```cpp
CombatComp->OnCombatRoleSystemReady.AddDynamic(this, &AMyController::OnReadyForCombat);

void AMyController::OnReadyForCombat(AAIController* Controller)
{
    UAICombatRoleSubsystem* Sub = GetWorld()->GetSubsystem<UAICombatRoleSubsystem>();
    Sub->ForceAssignRole(Controller, FGameplayTag::RequestGameplayTag("SEC.Role.Attacker"));
}
```
