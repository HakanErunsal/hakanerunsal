...
<UeSetPropertiesScene
  ...
  section="Weapon"
  rows={[
    { name: "Attach Socket", value: "hand_r", from: "hand_r", kind: "text" },
...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Socket Override", value: "None", from: "None", kind: "text" },
    { name: "Use As Weapon", value: "true", from: "true", kind: "bool" }
  ]}
  caption="One entry per thing the enemy carries. Leave Socket Override empty to use the socket the weapon names itself."
/>
...
Press Play. The weapon arrives in the enemy's hand, its trace sockets register, its animation layer links, and its sets reach the enemy. Run `SEC.Debug.Melee.DrawTracing 1` in the console to see the blade sweep.
...
**Use As Weapon** on the loadout entry decides whether any of the weapon reaches the enemy. With it off the weapon still spawns, attaches, and looks correct in the hand, while its moveset, its reactions and its animation layer go nowhere. One entry can be the weapon; mark two and the first one wins.
</Callout>
...
| **Drop** | Left in the level and asked to drop itself. Its **On Unequipped** event receives **Should Drop**, and turning on physics and setting a lifespan there is a couple of nodes. |
...
Death releases the enemy's equipment through the combat controller. **Drop Weapon On Death** on that controller is on by default and releases with **Drop**; turn it off and death releases with **Leave In World** instead. Reviving does not re-arm the enemy, so call **Equip Starting Loadout** on the way back.

**On Equipped Weapon Changed** fires on the server and on every client when the weapon changes, for a marker on the HUD or an ability that reads a socket. **On Equipped** and **On Unequipped** fire on the server for anything held.
...
- **A second entry took the weapon slot.** The first entry marked **Use As Weapon** wins, and the rest attach inert.
</Collapsible>
