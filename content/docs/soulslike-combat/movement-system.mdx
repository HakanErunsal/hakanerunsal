...
| Layer | When | How |
|-------|------|-----|
| **Tactical** | Target is within `HybridSwitchDistance` | Direct movement input for responsive strafing and positioning. Still reads the navmesh per direction for walkability, ledges, and hazards (see [Navmesh-Aware Sampling](#navmesh-aware-sampling)) |
...
### Approach-Blocked Signal (Cover-Break Hook)

The detour logic already computes whether the AI can see its target and whether a full navmesh route to it exists, then discards both. This signal exposes them, so you can react when a target is walled off behind something solid. That is the trigger for cover-break behavior: a player hides behind a breakable, the AI detects the dead end and destroys it. Server-only, additive, no change to movement decisions.

Read the state through these Blueprint-pure getters on `MovementEvaluatorComponent`:

| Getter | Meaning |
|--------|---------|
| `IsApproachBlocked()` | Sight blocked **and** no full route: the target is walled off. The cover-break gate. |
| `HasLineOfSightToTarget()` | Clear view to the target at the last detour recheck. Reports true (assumed clear) when out of range or not rechecking. |
| `IsTargetReachable()` | A full navmesh route to the target exists. Only refreshed while sight is blocked, so read it through `IsApproachBlocked`, not on its own. |
| `IsApproachStalled()` | Tactical steering is grinding cover without gaining ground. Transient, tactical-range only. |

And bind these delegates:

| Event | Fires |
|-------|-------|
| `OnTargetLineOfSightChanged` | Line of sight flips, clear to blocked or back. The parameter carries the new value. |
| `OnApproachBlocked` | Rising edge only: once when the target becomes walled off. |

The signal refreshes on the detour cadence (`DetourRecheckInterval`, default 0.5s), inside `HybridSwitchDistance`, while `bEnableDetourEscalation` is on. It is a throttled tactical-range read, so debounce a timer on bind rather than expecting a per-frame edge.

**Cover-break pattern (no extra C++):** bind `OnTargetLineOfSightChanged` and start a timer when it reports `false`. On timer expiry, gate on `IsApproachBlocked()`; if it is still true, line-trace from the pawn to the player on your cover channel and the first blocking hit is the obstacle to focus or attack. Clear the timer when `OnTargetLineOfSightChanged` reports `true`. `ResetApproachBlockedState()` clears the signal by hand; it also runs on target loss, detour disable, owner death, and teardown.
...
| `OnTargetLineOfSightChanged` | Detour-recheck view to the target flips (carries the new value) |
| `OnApproachBlocked` | Target becomes walled off, sight blocked and no full route (rising edge) |
