...
| Component | Purpose |
|-----------|---------|
| `AbilitySystemComponent` | Runs the attack abilities |
| `SECMeleeTraceComponent` | Frame-rate independent melee hits |
| `SECActionSetComponent` | Holds and replicates the action set |
| `SECReactionSetComponent` | Holds and replicates the reaction set |
| `SECCombatRoleComponent` | Replicates the assigned combat role to clients |
...
3. You inherit all eight combat components, the committed-attack rotation lock, smooth focus, and awareness-based facing.
...
- Override **Event_MontageNotifyBegin** and **Event_MontageNotifyEnd** to run logic at your notify windows: enable the melee trace, spawn VFX, apply effects.
...
`FinalScore = SelectionWeight × Distance × Angle × Health × TagMods × Novelty × Chain × (1 / RiskPenalty)`

Every factor is opt-in. A dimension only counts when you add its scorer:

- **Distance / Angle / Health / Speed Scorer.** Score against an `FRangeEval` sweet spot.
- **Attribute Scorer.** Score against any GAS attribute (mana, rage).
- **Attribute Gate / Stamina Gate.** Hard veto below a threshold.

Actions scoring zero or failing a gate are skipped. Chaining (`Preferred Follow-Up Action` + `Chain Bonus Multiplier`) boosts a combo follow-up once the current action ends.
</Collapsible>
...
- **Default State Tree** → leave empty to run the native combat loop (build context, move, then pick and run the best action each tick). Set it to `Content/SoulslikeEnemyCombat/Showcase/AI/StateTree/StateTree_SEC_Core` to drive the AI with that StateTree instead.
...
Turn on the component debug flags to watch the AI decide. On `MovementEvaluatorComponent`, `bDebugDrawScoring` draws the scored movement directions; on the action side, the score breakdown reports each factor per action. Enable these in the editor only, never in a shipping build.
</Collapsible>
