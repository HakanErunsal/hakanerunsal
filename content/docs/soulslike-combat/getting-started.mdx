...
- `SECActionSetComponent` for action set syncing and replication
...
> **Alternative:** You do not have to inherit from `EnemyCharacterBase`. Any `ACharacter` works. Implement `IEnemyAIConfigProvider` on your pawn so the controller can query for per-pawn AI settings. Add the components you need manually.
