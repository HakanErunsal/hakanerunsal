...
<Callout type="info">
Runs on Unreal Engine **5.6, 5.7, and 5.8**. Works in single-player and multiplayer (listen and dedicated server). Current plugin version: **3.0.0**.
</Callout>
...
The plugin depends on five engine plugins and turns them on for you: **GameplayAbilities**, **StateTree**, **GameplayStateTree**, **MotionWarping**, and **EnhancedInput**. Check **Edit → Plugins** to confirm they are enabled. MotionWarping drives the built-in attack warping, and EnhancedInput is used only by the showcase's demo player.
...
Reparenting hands you the whole set in one step, so reach for it whenever your character can inherit from `EnemyCharacterBase`. It also initialises the ability system for you, which is the part a Blueprint cannot do on its own: `InitAbilityActorInfo` is C++ only, and an ability whose ability system never received it refuses to activate without saying why.
</Collapsible>
...
1. **Implement the interfaces you need:** `IEnemyAIConfigProvider` and `ISECDamageable`. `IAbilitySystemInterface` is optional; the plugin finds an ability system through it when present and by component search otherwise.
...
3. **Initialise the ability system** in `PostInitializeComponents`, calling `InitAbilityActorInfo(this, this)`. Mark it replicated in the constructor with `SetIsReplicated(true)` and a `Minimal` replication mode so its cues and tags reach clients. Skip this and every ability refuses to activate.
...
The base also disables actor tick (`PrimaryActorTick.bCanEverTick = false`); the combat components run their own updates, so the pawn needs no tick. `ISECDamageable::HandleIncomingDamage` receives structured hits from the melee trace pipeline; override it to apply damage the way your game needs. Skip the interface and the actor still takes damage through Unreal's `ApplyPointDamage` fallback.
</Collapsible>
...
| **Cooldown → Cooldown Duration** | `2.0` |
