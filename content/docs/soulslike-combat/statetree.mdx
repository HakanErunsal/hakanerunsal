...
<Callout type="info">
  The graph **Recovery** state is a timed pause in the tree. [Action Recovery Time](/docs/soulslike-combat/action-system#recovery-a-gap-between-any-two-moves) on **`EnemyAIConfig`** only suspends offensive action selection while movement and reactions continue. Use Recovery for a brief settle; use **`ActionRecoveryTime`** to tune attack spacing.
</Callout>
...
**`SECCombatControllerComponent`** calls **`SyncStateForCombatRole`** when combat roles change (action set, reaction set, the equipped weapon's sets, movement profile). You only need the Sync tasks for custom controllers or when the graph must own sync timing.
...
**ASC Has Tag** binds **`Pawn`**, not the controller. The pawn supplies its ability system through **`IAbilitySystemInterface`** or by carrying an **Ability System Component**; the condition finds either.
