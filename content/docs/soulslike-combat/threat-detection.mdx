...
`AEnemyControllerBase` adds `ThreatDetectionComponent` in its constructor and leaves ticking off until `SECCombatControllerComponent` enables it on BeginPlay.
...
**Enable Threat Detection** defaults to on; BeginPlay calls `SetThreatDetectionEnabled(true)`. Uncheck it to stop ticking. Delegates still bind. `EnemyControllerBase` creates the component disabled; this flag turns ticking back on.
...
| [Combat Roles](/docs/soulslike-combat/combat-roles) | Each role can use a movement profile with its own threat toggles. |
...
| `SetThreatDetectionEnabled` / `IsThreatDetectionEnabled` | Toggle ticking; disabling clears threat state |
