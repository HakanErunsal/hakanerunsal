...
<Collapsible title="Letting a stare bias which move an enemy picks" badge="C++">
No shipped scorer reads this component. To let threat shape action scoring, subclass the scorer base and read the level off the controller:
...
Add it to an action's **Scorers** list like any other. You author the curve. See [Action System](/docs/soulslike-combat/action-system).

The scoring snapshot carries no threat figure. The one the debug overlay prints lives on the decision context that feeds the brain, blended from distance and angle to the target, and never reaches a scorer. Read the component when you want camera attention.
</Collapsible>
