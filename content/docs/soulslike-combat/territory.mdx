...
AActor*  AdvanceRoute();                              // step the cursor under PostOrder
...
bool     HasArrivedAt(const FVector& Location) const;
bool     HasArrivedAtGround() const;
FVector  GetGroundLocation() const;

// Leash readings.
FVector  GetLeashAnchorLocation() const;
AActor*  GetLeashAnchorActor() const;
bool     HasLeash() const;
float    GetDistanceOutsideLeash(const FVector& Location) const;
...
`USECTerritoryPostComponent` holds what a post is tuned by, and `USECTerritoryPostComponent::FindPostComponent(Actor)` resolves it from any actor. Override `PickPointInArea` in a Blueprint or C++ subclass of that component for a post that works its destination out at runtime; the base picks among reachable spots inside the area under `WanderSelection`.

`USECTerritoryComponent::GetPostLocationFor(Post)` and `GetWanderExtentFor(Post)` answer for any actor, whether it carries a post component or not. `USECTerritoryComponent::FindTerritoryComponent(Controller)` resolves the component from a controller, checking its possessed pawn first.
