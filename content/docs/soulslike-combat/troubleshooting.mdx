...
Press Play and turn the overlays on. Each writes a block above the enemy it belongs to.
...
<Callout type="info">
**Decision**, **Movement**, **Scoring**, **Awareness**, **Territory** and **Tokens** are worked out on the server and draw only there. **Role**, **Actions**, **Reactions**, **Vitals** and **Teams** replicate, so they read on a client too.
</Callout>

<Collapsible title="Debug one enemy from the Details panel" badge="Advanced">
Every console switch has a matching checkbox on the component it belongs to, and ticking one there affects that enemy alone. The overlay sections have a **Debug Overlay** checkbox each, in the same place.
...
2. **Target Selector is empty** on the Enemy AI Config. Set it to **Closest Target Selector**. An enemy that registered before its target appeared gets nothing without one.
3. **Awareness Config is empty** on the Enemy AI Config. The enemy senses nothing, holds at Unknown, and never enters its combat phase. Point it at an Awareness Config asset and leave that asset's defaults alone.
...
| **Recovery** | The gap after any action. Set the three **Recovery** fields on the AI Config to 0 to turn it off. |
...
- **The Montage Payload has no montage.** Saving the Action Set reports this in the **Soulslike Enemy Combat** message log.
...
Call **Handle Revive** on the **SEC Combat Controller** component when the enemy comes back. It releases the block, restarts the brain, and turns threat detection, perception and role registration back on. Re-equip the weapon yourself, since death dropped it.
...
- Set **Ability Timeout** on the action to bound the wait.
...
Two things quietly defeat it. A token tag with no row in Project Settings hands out unlimited slots, and an enemy with no focus target claims nothing and runs uncapped. `SEC.Debug.Tokens 1` prints `unlimited` in place of a count for the first case.
...
- **No navmesh.** Past 800 cm the enemy asks for a path, the request fails every tick, and it never closes the gap. Add a **NavMesh Bounds Volume**, build navigation, and press **P** in the viewport to check it covers the floor.
...
**Drop Weapon On Death** is on by default and unequips with physics. Untick it to handle the drop yourself, for example from a death montage notify.
