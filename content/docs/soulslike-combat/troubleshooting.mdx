...
2. **Target Selector is empty** on the Enemy AI Config. Set it to **Awareness Filtered Target Selector** and leave its **Inner Selector** empty, so it offers the nearest target this enemy has perceived. An enemy that registered before its target appeared gets nothing without a selector. **Closest Target Selector** gets it moving too, but it engages anything registered whether the enemy sensed it or not.
