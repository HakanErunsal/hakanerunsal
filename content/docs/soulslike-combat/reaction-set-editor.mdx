...
Double-click any Reaction Set and it opens as cards, under a tab called **Triggers and Reactions**. If you have none yet, right-click in the Content Browser, **SEC**, **Reaction Set**.
...
An attack that does not announce itself reaches no rule, so the canvas has nothing to report however the cards are wired. Add the **SEC Attack Telegraph** notify to the attacker's montage where the wind-up starts. The **Watching** picker fills from AI whose active Reaction Set is this asset, which they reach through their **Enemy AI Config**, so a picker stuck on **None** while the game runs is one of those two. See [Attack Telegraphs](/docs/soulslike-combat/attack-telegraphs) and [Reaction System](/docs/soulslike-combat/reaction-system).
</Callout>
...
Copy, cut, paste and duplicate work as they do in any Unreal graph. A pasted card gets a fresh ID and its own copies of the original's scorers, gates and execution method, so editing the copy leaves the original alone. Right-click also offers **Add Comment**, which groups cards under a label and changes nothing about how the AI runs.
...
<Callout type="warning">
A rule with a category set and an arrow to a reaction outside that category answers nothing at all. The trigger card flags it and names both the reaction and the category. Clear the category, or give that reaction the category.
</Callout>
