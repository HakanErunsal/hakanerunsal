...
A new trigger is asked before the rules already there, on the assumption that a rule you have only now written is the one you are trying out. Change its **Rule Priority** to move it.
...
Each arrow carries a **Weight Multiplier** on the link, saying how much this rule favours that answer over the others it names. Leave it at 1 and the reaction's own **Selection Weight** decides. A thicker, brighter arrow is a rule leaning harder on that answer.
