...
> **Custom character classes:** `ActionEvaluationComponent` resolves the `AbilitySystemComponent` from the possessed pawn via `IAbilitySystemInterface` on controller possession. If your pawn does not implement this interface, ability-based actions will silently fail. See [Getting Started](/docs/soulslike-combat/getting-started) for setup details.
---
