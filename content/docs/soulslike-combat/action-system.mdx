...
**Scorers** fold in as multipliers, one per dimension you opt into. Leave a dimension off and it does not affect the score: an action with no Distance Scorer scores the same at any range. A scorer that returns **0** zeroes the whole product for that action. The full set of built-ins is in [Scorers & Gates](#scorers--gates).
...
Wiring these on a canvas, and watching a chain fire, is what the [Action Set Editor](/docs/soulslike-combat/action-set-editor#building-a-combo) is for.
...
| 2 | **Provided Set** | A set pushed onto the pawn, which is how an equipped weapon's moveset arrives. |
...
<Protip 
  ...
  description="A weapon's own set outranks Config. Give a skeleton a Bow and it becomes a sniper. Take the bow away and it falls back to its Config set as a brawler." 
/>

### Giving a weapon its own moveset

Fill in **`Weapon Action Set`** on the weapon Blueprint. While the enemy holds that weapon it fights from that set, and releasing the weapon hands the enemy back to its AI Config. See [Weapons](/docs/soulslike-combat/weapons) for how the weapon reaches the enemy.
...
<Collapsible title="Pushing a set without a weapon" badge="C++">
...
| **Combat Token Gate** | Gate | `TokenTag`, `Cost` (default `1`) | Veto unless the target has a free permission slot, then hold that slot until the action ends. Caps how many enemies run the action against one target at once. See [Combat Tokens](/docs/soulslike-combat/combat-tokens). |

<Callout type="info">
  The decision context does not read GAS attributes. Health Scorer and Stamina Gate read the snapshot sourced from the pawn's vitals, defaulting to full health and 100 stamina when the pawn carries no vitals component or no row for the configured tag. For an attribute, reach for **Attribute Scorer** or **Attribute Gate**; for a specific pool, a [Vital Scorer or Vital Gate](/docs/soulslike-combat/vitals#costing-and-scoring-actions).
</Callout>
...
**Let one enemy swing at a time.** Add a **Combat Token Gate** to every melee attack, set `TokenTag = SEC.Token.Attack`, and drop that pool to 1 in Project Settings. The rest circle until the slot frees. See [Combat Tokens](/docs/soulslike-combat/combat-tokens).
...
To watch an enemy score these actions while the game runs, open its Action Set and press Play: the [Action Set Editor](/docs/soulslike-combat/action-set-editor#watching-the-ai-decide) lights each action up with its live score, its cooldown, and the gate that refused it.
