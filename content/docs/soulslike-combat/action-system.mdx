...
> Scores every action against live battlefield data: distance, angle, health, threat, and context tags.
...
**Scorers** (distance, angle, health, speed, and custom ones) fold in as multipliers. Leave them off and that dimension does not affect the score. An action with no Distance Scorer scores the same at any range.
...
An action commits its cooldown only **after** the ability activates. If a [`CanActivateAbility`](#contextual-execution) override (or any other GAS gate) refuses activation, the action stays off cooldown and can retry next tick.
...
**Require Line Of Sight** reads `FDecisionContext::bHasLOS`. The [Build Decision Context](/docs/soulslike-combat/statetree#sttask_builddecisioncontext) task traces from the AI's eyes to the target each tick. Tick the box for ranged shots and gap-closers. Leave it off for moves that land blind (radial slam, taunt).
...
The same Scorers and Gates work on reactions, with the snapshot caveat below. See [Reaction System](/docs/soulslike-combat/reaction-system#scorers--gates).
...
<Callout type="warning">
  Keep scorer and gate subclasses **stateless**. One instance is shared across every AI using the asset, so mutable fields alias across enemies. For randomness, use the `SeededRandom` helper.
</Callout>
...
These fields fill on the **action path only**. On the reaction path they hold defaults (`0` / `0` / `0` / `1.0` / `100`), so distance, angle, health, speed scorers and the Stamina Gate do nothing useful on reactions. Score reactions with priority, weight, tags, and Attribute Scorers or Gates.
