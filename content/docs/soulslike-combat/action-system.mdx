...
| **Chain Only** | the follow-up | Makes the link a requirement rather than a preference: automatic selection reaches that action only inside the chain window. Good for a finisher that should never open a fight. |
...
| **Chain Only** | The chain window is shut, so no link can reach it. |
