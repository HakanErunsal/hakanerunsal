...
| Field | Sets |
|-------|------|
| **Ability Class** | The ability this action runs. |
| **Ability Timeout** | Safety limit in seconds for one run. 0 leaves the ability's own end as the only limit, backed by a 30 second safety. Advanced. |

Two siblings cover the cases a class does not:

| Method | Use it for |
|--------|-----------|
| **Gameplay Ability By Tag** | An ability granted somewhere else. Takes an Activation Tag instead of a class, and finishes when that ability ends. |
...
| **Payload** | Data the action hands to its trees. Leave it None for a sequence that needs nothing. Pick **Ability Payload** to name a gameplay ability, which SEC grants so an Activate Blackboard Ability node can start it. |
| **Payload Key Name** | Blackboard key the payload is written to, `SEC_Payload` by default. Shown once a payload is set. Advanced. |
