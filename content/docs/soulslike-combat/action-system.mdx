...
Recovery time is a server-side breather. After an enemy commits an action, you can suspend its offensive action selection for a tunable number of seconds. While the enemy recovers it still strafes, repositions, and reacts (parry, dodge, counter), it just does not start a new attack. Set it to space out a relentless attacker into readable bursts.
...
You tune the global windows on `UEnemyAIConfig` under the **Recovery** category. Every field defaults to `0`, so recovery is off until you set one and existing projects behave exactly as before.
...
2.  **Define Actions:** Each entry needs an `ActionId`, `SelectionWeight`, an execution mode, and (for range behavior) a Distance Scorer in its Scoring list. A new action with no Distance Scorer is distance-agnostic.
