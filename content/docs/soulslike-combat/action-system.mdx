...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Montage", value: "AM_Attack_Light", kind: "asset" }
  ]}
...
Add an entry to an action's **Chain Links** naming the follow-up it prefers and the **Bonus Multiplier** that follow-up gets, 1.5 by default. After the first move completes, the named follow-up scores higher for a short window. One action can prefer several follow-ups, each with its own bonus.

Follow-ups can fire during the recovery window, for **Chain Followup Window Seconds** after the action ends (0.6 by default, on the Action Evaluation component). Only the actions its chain links name get that pass. An interrupt or a cancel clears it, so a parried enemy cannot combo through its own recovery.
...
Wiring this on a canvas and watching it fire is what the [Action Set Editor](/docs/soulslike-combat/action-set-editor#building-a-combo) is for.
...
Saving the asset also runs validation, which reports into the **Soulslike Enemy Combat** message log listing and names any action that has nothing to run.
...
- **Enabled is unticked.** That also stops the ability being granted, so a later manual call has nothing to start. For a scripted boss move use **Manual Only** instead.
...
| **Manual Only** | It is reserved for a direct Execute Action call. |
...
| **Recovery** | The enemy is in a recovery window, chain follow-ups excepted. |
