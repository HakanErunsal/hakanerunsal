...
| **Chain Only** | the follow-up | Makes the link a requirement rather than a preference: automatic selection reaches that action only inside the window. Good for a finisher that should never open a fight. |
...
| **Chain Only** | Nothing that chains into it has just finished, so the window is shut. |
