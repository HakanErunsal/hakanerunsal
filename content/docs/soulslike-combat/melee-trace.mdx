...
6. On a target: **`BuildDamageInfo`** → damage → **Hit Effects** → **`HandleDamageResult`** → **`OnMeleeHitResponse`**. A downed combatant or the level takes no damage and reports an empty result.
...
| Volume in front of a character eats its hits | Sensor collider set to **Block** | Set that collider's response to the trace channel to **Overlap**, and list its component tag under **Sense Component Tags**. The log names the component. |
