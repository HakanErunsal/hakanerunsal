...
The `SkipRecordTags` container on the melee trace component allows you to **conditionally skip hit recording**. If the damage result's `ResponseTag` matches any tag in `SkipRecordTags`, the hit is not recorded, so the same actor can be hit again on the next trace tick.
...
<Protip
  ...
  description="For reactions to projectiles or area tells, use a sense-detection volume on the AI and call EvaluateBestReaction() from its overlap event. The melee trace damage path is one of many triggers; the reaction system is stimulus-agnostic."
/>
