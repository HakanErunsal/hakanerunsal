...
Subclass **`ASECWeaponBase`**, fill **`TraceSockets`**. Equipping the weapon points each socket at the weapon mesh and registers it on the wielder's trace component, and releasing the weapon takes those sockets away again.
...
An enemy has to be holding the weapon before any of the above runs. Put the weapon in the pawn's loadout and the plugin spawns it, attaches it, and registers its trace sockets.
...
That is the setup. The weapon arrives in the enemy's hand on spawn, its trace sockets register, its animation layer links, and its **`Weapon Action Set`** and **`Weapon Reaction Set`** reach the enemy.
...
| No hits | Sockets missing | Equip the weapon so its sockets register; the wielder needs a trace component. |
