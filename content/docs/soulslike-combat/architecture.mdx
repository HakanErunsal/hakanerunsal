...
`EnemyCharacterBase` and `EnemyControllerBase` ship with everything below already on them, so a new enemy starts complete and you delete what it does not need. [Step-by-Step Enemy](/docs/soulslike-combat/step-by-step-enemy) builds one from those two.
...
| **Awareness** | [Sight, hearing and damage memory](/docs/soulslike-combat/awareness-system), and the last-known location behind an investigation. |
...
| **SEC Territory** | The ground this enemy owns: [posts, a wander patch, a leash](/docs/soulslike-combat/territory). Inert until a post is named. |
| **SEC Team** | Team ID, free-for-all, and target exclusions. |

**SEC Combat Controller** is the one piece a setup cannot go without. The rest are optional: drop **Awareness** for an enemy that fights whatever it is handed, drop **Movement Evaluator** for one that holds its spot.
...
Adding **Awareness** is what unlocks Investigate; adding **SEC Territory** with at least one post is what unlocks Resting. Without either, the enemy sits in Idle until it is handed a target and jumps straight to Combat.
...
Run `SEC.Debug.WatchPawn` with no argument to list the pawns the overlay has something to say about. Part of a name is enough, and running it again on a watched pawn stops watching it.
...
<Callout type="info">
Role, Actions and Reactions replicate, so those three read on a client. The rest are worked out on the server and draw only there.
</Callout>
...
| Awareness | **Debug Overlay Awareness** on the controller's **Awareness** |
...
3. Add the ones that enemy needs: **SEC Brain**, **Movement Evaluator**, **Action Evaluation**, **Reaction Evaluation**, **Awareness**, **Threat Detection**.
...
| **SECMotion** | What moves a character: the approach component, its montage window, and the SEC Skew Warp modifier. The only module that needs the Motion Warping plugin. |
