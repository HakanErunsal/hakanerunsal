...
- Fill rate scales with **distance** and **angle**. A centred, close target fills fast; one at the cone edge and far away trickles up. Shape this with `DistanceRateCurve` and `AngleRateCurve` (flat by default). A per-sense [runtime multiplier](#tuning) stacks on top.
...
These live on `USECAwarenessComponent` itself, not the config asset, so each AI holds its own values.

| Item | Purpose |
|------|---------|
| `AwarenessRateMultipliers` | Per-sense map of multipliers on awareness gain. Sight scales the meter fill rate; hearing (and any sense mapped to Suspicious) scales the bump. No entry = 1 |
| `SetAwarenessRateMultiplier(Sense, Value)` | Set one entry at runtime; it holds until set again. Clamped at 0, and 0 stops that sense from raising awareness |

Senses that resolve to Detected (damage by default) and the proximity floor ignore the multipliers; a hit or a point-blank target still reveals. A scaled hearing bump can land below `SuspicionThreshold`, raising the meter without registering as Suspicious, so repeated quiet noises stack toward investigation.
...
### Recipe 6: Crouching halves detection

Two ways, by scope:

- **Per AI, no override.** Call `SetAwarenessRateMultiplier(AISense_Sight, 0.5)` on the awareness component. Good for archetypes: a sleepy guard at 0.5, a hawk-eyed elite at 2.0. The value holds until you set it again.
- **Per target.** Override `GetAwarenessRateMultiplier` in your controller Blueprint: if the Target is crouching, return 0.5; otherwise return the parent call. The AI now notices a crouching player at half speed and everyone else at full.

For sound, add an `AISense_Hearing` entry the same way, or scale loudness at the `ReportNoiseEvent` call site (that affects all listeners; the multiplier is per AI).
...
| `GetAwarenessRateMultiplier(Sense, Target)` | BlueprintNativeEvent | Scale one sense's awareness gain per target, e.g. return 0.5 while the target crouches. The default reads `AwarenessRateMultipliers` ([Tuning](#tuning)). |
