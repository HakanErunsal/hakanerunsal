...
- **Hearing** raises the meter to at least `SuspicionThreshold` (using `HearingBump` when it is higher), so sound always reaches Suspicious but never confirms a target on its own.
