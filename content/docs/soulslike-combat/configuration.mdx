...
Behavior
  StateTree: StateTree_SEC_Core   // optional; leave empty to run the native C++ combat loop
...
struct FRangeEval
{
    ...
    float Exponent;      // Falloff sharpness outside the sweet spot; 2.0 is smooth, 4.0 sharp (default 2.0)
    bool bClampToZero;   // Hold the score at zero outside the range rather than letting it go negative (default true)
};

The ramp between an edge and the sweet spot is a curve, not a step: the distance from the edge is raised to **Exponent**.
```
