...
No target selector ships turned on. With **Target Selector** empty on the Enemy AI Config and **Default Target Selector** empty in Project Settings, an enemy that registers while a target is already in the pool takes the first one it may fight, which hides the gap in a simple level. An enemy possessed or spawned before the player never picks one up and stands still forever. Fill in one of the two. [Targeting](/docs/soulslike-combat/targeting-system) covers the choices.
</Callout>
...
**Default State Tree** picks the brain. Leave it empty and the **Fallback State Tree** on the StateTree component answers instead; with that empty too, the enemy runs the plugin's own combat loop with no StateTree at all. `StateTree_SEC_Core` ships with the plugin as a graph to start from. See [StateTree](/docs/soulslike-combat/statetree).
