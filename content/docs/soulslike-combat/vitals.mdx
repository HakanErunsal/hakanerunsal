...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Regen Rate Per Second", value: "0.0", from: "0.0" }
  ]}
...
<UeSetPropertiesScene
  ...
  rows={[
    ...
    { name: "Max Value", value: "100.0", from: "100.0" },
...
A pool the pawn has no row for reads as nothing at all: a **Vital Gate** pointed at it passes, a **Vital Scorer** pointed at it scores flat, and health reads as full. Nothing logs. So a mistyped tag, or a health pool renamed without updating **Health Vital Tag** in Project Settings, leaves an enemy that cannot be gated and cannot die.
</Callout>
...
| **Depleted Gameplay Event** | Sent to the pawn once per depletion, on the server. |
...
Four nodes change a pool. Each runs on the server; a client call is ignored with a warning.
...
<Protip
  ...
  description="Handle Revive puts a dead enemy back under AI control. Re-arm it afterwards with Equip Starting Loadout, since death released the weapon."
/>
...
- **The tag has no row.** Mutators do nothing and readers answer 0. **Has Vital** tells you in one node.
