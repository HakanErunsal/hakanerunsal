...
| Delegate | Fires |
|----------|-------|
| **`OnVitalChanged`** | Every discrete change: a spend, damage, a set or fill, or regeneration completing at max. Regeneration's smooth rise between those points fires nothing; poll `GetVitalValue` for a smooth bar. |
| **`OnVitalDepleted`** | Once per depletion episode. **Depleted Loose Tag** is already applied when it fires. |
| **`OnVitalFull`** | When the pool reaches its maximum, by a gain or by regeneration. |
