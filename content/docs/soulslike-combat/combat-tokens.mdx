...
`USECTokenGate::CollectRequirements(const FSECCustomScoring&, TArray<FSECTokenRequirement>&)` gathers what a spec's gates need, merging duplicate tags and skipping inverted gates. `UActionEvaluationComponent` calls it at the start of every action and claims against the controller's focus actor, the same actor the gate scored against.
