...
|-----------|-----------------|
| `SECActionSetComponent` | Current action ID, execution state, active ActionSet reference |
...
The controller pushes execution events to the pawn's `SECActionSetComponent`, which replicates them:
...
The role subsystem assigns roles to controllers (server-only). When a role changes, the controller writes it to the pawn's `SECCombatRoleComponent`, which replicates the tag:
