...
> The plugin works in listen server and dedicated server configurations. AI decisions run on the server; relevant state replicates to clients.

## Overview

AI in Unreal Engine is server-authoritative. AI controllers only exist on the server, and all decision-making happens there. The challenge is giving clients enough information to display what the AI is doing (animations, VFX, UI indicators) without duplicating logic.

The approach: **replicate output state**, not logic. The server runs the full decision pipeline, and lightweight replicated components on the pawn broadcast the results to clients.
