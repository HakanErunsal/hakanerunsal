...
- `USECActionSetComponent` - Action set resolution and replicated action state
- `USECCombatRoleComponent` - Replicated combat role for client visibility

- `AIConfig` -> Set it to your AI Config or the `DA_SEC_Enemy_AIConfig_Default`

>AIConfig will be acquired from the pawn by the controller's `SECCombatControllerComponent` if it's set here. If it is not set in the pawn, the component will use its own `DefaultAIConfig`. If neither is set, there will be no AI behavior.
...
UBotStateTreeAIComponent*       // StateTree AI behavior
```
...
Threat-to-movement response is auto-wired if both `ThreatDetectionComponent` and `MovementEvaluatorComponent` are present. Configure `bSwapStrafeOnHighThreat` and `bAdjustDistanceByThreat` on the `SECCombatControllerComponent`.

For advanced customization, subclass `SECCombatControllerComponent` in Blueprint and override `SyncStateForCombatRole` (custom role-change behavior) or `HandleCombatTargetLost` (custom target-loss fallback). Subclass `ActionEvaluationComponent` and override `CanExecuteAction` or `ModifyActionScore` for custom action gating and scoring.
