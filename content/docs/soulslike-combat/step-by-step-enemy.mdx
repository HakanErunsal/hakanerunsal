...
The attacks play, but nothing lands until the weapon runs melee traces. The full weapon build lives on the [Melee Trace](/docs/soulslike-combat/melee-trace) page: inherit `ASECWeaponBase`, define the trace sockets, add the `SECMeleeTraceWindow` notify to each montage, and assign a damage config. Set that up, then come back to test.
