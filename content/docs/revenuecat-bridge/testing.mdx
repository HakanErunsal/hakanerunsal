...
description: "16-phase test plan, edge cases, logging tips, and common issues for RevenueCat Bridge."
date: 2026-03-14
published: true
...
Verify all of this is in place before testing:
...
For a quick confidence check, run these in order:
...
The real-world flow: the game launches and you need subscriber status immediately.
...
Tests the native paywall UI. Requires iOS 16+ or Android with Compose.

| # | Test | Expected | Log to check |
|---|------|----------|--------------|
| 15.1 | Call `PresentPaywall()` with empty OfferingID | Native paywall appears showing the current offering | - |
| 15.2 | Complete a purchase through the paywall | `OnPaywallCompleted` fires with `Purchased` result and updated `CustomerInfo` | `[RevenueCat] Paywall completed` |
| 15.3 | Check `IsEntitlementActive()` after paywall purchase | Returns `true` | - |
| 15.4 | Dismiss the paywall without purchasing | `OnPaywallCompleted` fires with `Cancelled` result | - |
| 15.5 | Tap "Restore" in the paywall | `OnPaywallCompleted` fires with `RestoreCompleted` result | - |
| 15.6 | Call `PresentPaywall("specific_offering")` | Paywall appears showing the specified offering's products | - |
| 15.7 | Call `PresentPaywallIfNeeded("pro")` without entitlement | Paywall appears | - |
| 15.8 | Call `PresentPaywallIfNeeded("pro")` with active entitlement | Nothing happens, no paywall shown | - |
| 15.9 | Use the **Present Paywall (Async)** Blueprint node | Output pins fire correctly (OnPurchased / OnCancelled / OnRestored / OnFailure) | - |
| 15.10 | Verify paywall on Android | Compose-based paywall renders correctly | Check Logcat for Compose errors |
| 15.11 | Verify paywall on iOS | SwiftUI-based paywall renders correctly | Check Xcode console |

---

### Phase 16 - Virtual Currencies

Needs virtual currencies configured in the RevenueCat dashboard, with a product that grants currency on purchase.

| # | Test | Expected | Log to check |
|---|------|----------|--------------|
| 16.1 | Call `FetchVirtualCurrencies()` | `OnSuccess` fires (and `OnVirtualCurrenciesUpdated`) | `[RevenueCat] Fetched N virtual currencies.` |
| 16.2 | Inspect a currency | `Code`, `Name`, and `Balance` populated for each dashboard currency | Log each code and balance |
| 16.3 | Call `GetVirtualCurrencyBalance("GLD")` | Returns the same balance as the fetched data | - |
| 16.4 | Buy a product that grants currency, with `bAutoFetchVirtualCurrencies` on | Balance rises after the purchase with no manual fetch | - |
| 16.5 | Grant currency through the RevenueCat backend, then call `InvalidateVirtualCurrenciesCache()` and `FetchVirtualCurrencies()` | New balance returns | - |
| 16.6 | Call `FetchVirtualCurrencies()` with no dashboard currencies | Empty array, no error | - |

---

## Edge Cases

Run these once the happy path passes.
...
| # | Test | Expected |
|---|------|----------|
| E23 | Call `PresentPaywall()` on Win64 editor | Stub log message, no crash |
| E24 | Call `PresentPaywall()` on iOS < 16.0 | Graceful fallback or warning, no crash |
| E25 | Call `PresentPaywall("nonexistent_offering")` | Error result or falls back to current offering |
| E26 | Call `PresentPaywall()` while another paywall is already showing | No duplicate paywall, no crash |
| E27 | Call `PresentPaywallIfNeeded("")` (empty entitlement) | Warning log, no crash |
| E28 | Paywall appears, player switches apps, returns | Paywall still visible or result fires |
| E29 | No network, present paywall | Error fires or cached data shown |
