...
| **Auto Fetch Offerings** | On by default. After the SDK configures, the subsystem calls `FetchOfferings()` once so `GetCachedOfferings` has data before you ask. |
| **Auto Fetch Customer Info** | On by default (v1.3.0+). After the SDK configures, the subsystem calls `FetchCustomerInfo()` once to populate the customer-info cache. Safety net for cold launches where the native SDK doesn't populate it. Bind `OnCustomerInfoUpdated` to apply entitlements. Bind `OnError` on failure. |
| **Auto Fetch Virtual Currencies** | Off by default (v1.6.0+). When on, the subsystem fetches virtual currency balances after configure and refreshes them after each purchase. Turn it on if your project uses virtual currencies, then bind `OnVirtualCurrenciesUpdated` to read them. |
...
- [Testing & Troubleshooting](/docs/revenuecat-bridge/testing) - the full 16-phase test plan, edge cases, and logging tips
